	...
	if response.status_code == 200:
		...
		for payload in msg.get_payload():
			if payload.get_content_type() == "application/json":
				...
				logger.info("JSON String Returned: %s", json.dumps(j, indent=2))
...
def trigger_process(trigger):
	...
	if trigger.voice_confirm:
		...
		logger.info("Voice has been confirmed to perform.")
...
def alexa_talker():
	...
	pub = rospy.Publisher('alexa_input', String, queue_size=10)
	rospy.init_node('alexa_avs', anonymous=True)
...
if __name__ == "__main__":
	...
	if not silent:
		...
		logger.info("Receptioninst Ever ready to deploy!")

	# Produce ros node module
	# alexa_talker()
